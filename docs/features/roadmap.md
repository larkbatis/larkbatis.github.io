...
| | Scope | |
|---|---|---|
| **M0** | Benchmark groundwork — an `ObjectWrapperFactory` experiment on mybatis-3 itself, to test the central claim before writing any LightBatis code | :material-check: |
...
| **M5** | Extended benchmarks, native-image smoke test, legacy-mapper scanner | :material-progress-clock: in progress |
...
and `MetaObject` from the row-read path measurably improved things, LightBatis's central
...
| | Status |
|---|---|
| `lightbatis-benchmarks` (JMH; pinned sessions; STATEMENT-scope cache; H2 over TCP; JDK 17 vs 21; 50-bean megamorphic experiment) | Implemented; the published results table is not final |
| `lightbatis-scanner` (`lightbatis-scan`) | Implemented. [Details](migration.md) |
...
!!! warning "The native-image promise is unverified"
    ...
    It is the project's strongest qualitative claim, and it is structural — there is no
    `Proxy`, no `Class.forName`, no `setAccessible` to declare, and you can check that by
    reading the code. But **no real native-image build has been done**. It should not be
...
| | Why |
|---|---|
| **Multiple `DataSource`s** (`@LightBatisDataSource`) | No design without a real service that needs it. Today: one `SpringLightBatisSession` per `DataSource`, mapper `@Bean` methods written by hand |
| **`@Column`, `@Handler`, `@LightBatisRow`** | Declared in the annotations artifact; the processor does not read them yet. [Details](annotations.md#declared-but-not-yet-implemented) |
...
These are not backlog items. They are on the wrong side of the
