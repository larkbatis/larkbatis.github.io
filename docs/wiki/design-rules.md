...
could not be parsed; `SqlFragment` contents; `databaseId`, chosen once at startup.
...
No `Proxy`, no `Class.forName`, no `setAccessible` — in the runtime module or in generated
code. `lightbatis-runtime` keeps zero dependencies beyond JDBC.
...
`lightbatis-processor` and both build plugins are compile-time tools.
...
`${}` get a generated `LightBatisSql.trackVariants` call.
...
`LightBatisSession.conn()` goes through `DataSourceUtils`, never
...
The design document lists four objections it considers serious, and answers rather than
dismisses them:
...
| *"Build time will go up."* | Yes, and it is a real cost paid by developers daily rather than by production. Mitigated by incremental processing. But it is moving the cost left, not deleting it |
...
There is also a fifth, on codegen bloating jars and IDEs: one impl and one reader per
